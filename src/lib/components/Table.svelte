		...
		<table class="table table-hover">
			...
			<tbody>
				{#each $exams as exam, index}
					<tr class="">
						<td><p class="py-2 max-w-[110px] lg:max-w-full truncate">{exam.name}</p></td>

						<td class=""><p class="min-w-[50px] py-2">{exam.cfu}</p></td>
						<td class=""><p class=" py-2">{exam.vote}</p> </td>
						<td class="max-w-[110px] lg:max-w-full">
							<button class="btn btn-sm" on:click={() => onEditExamClicked(exam, index)}
								><svg
